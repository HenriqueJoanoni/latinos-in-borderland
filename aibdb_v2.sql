...
SELECT g.objective AS "Game Objective",	g.card_type AS "Card Game Type", g.difficulty AS "Game Difficulty",	g.player_count AS "Player Count", p.player_name AS "Player Name", p.player_age AS "Age",
	...
	CASE WHEN dl.player_id IS NOT NULL THEN 'Dead' ELSE 'Alive' END AS "Status",
	dl.death_reason AS "Death By", il.injury_type AS "Injured By", il.severity AS "Severity"
	FROM game g
		LEFT JOIN player p ON g.player_id = p.player_id
		LEFT JOIN game_master gm ON	g.game_master_id = gm.game_master_id
			AND gm.player_id = p.player_id
		LEFT JOIN death_log dl ON g.game_id = dl.game_id
			AND g.player_id = dl.player_id
		LEFT JOIN injury_log il ON g.game_id = il.game_id
			AND g.player_id = il.player_id
		ORDER BY g.objective;


/** UPDATE STATEMENTS */
UPDATE player SET player_age = 34 where player_id = 26;
UPDATE game SET player_count = 15 where game_id = 1;
UPDATE game SET	game_master_id =
	CASE
		WHEN game_master_id = 16 THEN 3
		WHEN game_master_id = 11 THEN 4
	END
...
DELETE FROM player WHERE player_id = 26;
...
CREATE PROCEDURE LocationByGame(IN loc_id INT,IN game_id INT)
BEGIN
    SELECT a.location_name AS "Location", a.location_type AS "Location Type", b.objective AS "Objective", b.difficulty AS "Difficulty",
    		c.player_name as "Game Master"
		FROM location a
			JOIN game b ON a.game_id = b.game_id
			JOIN game_master gm ON b.game_master_id = gm.game_master_id
			JOIN player c ON gm.player_id = c.player_id
